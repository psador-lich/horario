# Horario de Professores v4
# This notebook implements data loading from a CSV file and a fallback to example data.
...
# 3. Funções de Geração e Carregamento de Dados
...
def load_data_from_csv(filepath):
    ...
    required_columns = ['curso_nome', 'turma_nome', 'turma_turno', 'turma_alunos',
                        'disciplina_nome', 'disciplina_duracao', 'professor_nome', 'sala_requerida']
    if not all(col in df.columns for col in required_columns):
        print(f"Erro: O arquivo CSV não contém todas as colunas necessárias. Colunas necessárias: {required_columns}")
        return None, None, None, None

    if df.isnull().values.any():
        print("Aviso: O arquivo CSV contém dados faltantes. Linhas com dados faltantes serão ignoradas.")
    ...
    for _, row in df.iterrows():
        disc_key = (row['disciplina_nome'], row['curso_nome'], int(row['disciplina_duracao']), row['sala_requerida'])
        if disc_key not in disciplinas_cache:
            disciplinas_cache[disc_key] = Disciplina(
                nome=row['disciplina_nome'], curso=row['curso_nome'],
                duracao=int(row['disciplina_duracao']), tipo_sala_requerida=row['sala_requerida']
            )
        disciplina = disciplinas_cache[disc_key]

        if row['professor_nome'] not in professores:
            professores[row['professor_nome']] = Professor(row['professor_nome'], [], horarios_indisponiveis=[])
        if not any(d.nome == disciplina.nome for d in professores[row['professor_nome']].disciplinas_que_leciona):
            professores[row['professor_nome']].disciplinas_que_leciona.append(disciplina)

        if row['turma_nome'] not in turmas:
            turmas[row['turma_nome']] = Turma(
                nome=row['turma_nome'], numero_de_alunos=int(row['turma_alunos']),
                disciplinas=[], turno=row['turma_turno']
            )
        turmas[row['turma_nome']].disciplinas.append(disciplina)

    salas = {}
    tipos_sala_requeridos = df['sala_requerida'].unique()
    for tipo in tipos_sala_requeridos:
        if tipo != 'comum':
            for i in range(1, 6):
                salas[f"{tipo}_{i}"] = Sala(f"{tipo}_{i}", 25, True, tipo_sala=tipo)
    for i in range(1, 26):
        salas[f"Sala Comum {i}"] = Sala(f"Sala Comum {i}", 35, False, tipo_sala='comum')

    print("\n--- Resumo dos Dados Carregados ---")
    print(f"Total de Cursos: {df['curso_nome'].nunique()}, Turmas: {len(turmas)}, Professores: {len(professores)}")
    print("------------------------------------")
    print("Confirmação do usuário para continuar com os dados carregados.")
    return list(turmas.values()), list(professores.values()), list(salas.values()), []

def generate_example_data():
    print("Nenhum arquivo CSV fornecido. Usando dados de exemplo gerados aleatoriamente.")
    restricoes_globais = []
    disciplina_names = [
...
# csv_filepath = None
csv_filepath = "dados_horario.csv"

turmas, professores, salas, restricoes_globais = [], [], [], []
if csv_filepath:
    turmas, professores, salas, restricoes_globais = load_data_from_csv(csv_filepath)
    if turmas is None:
        print("Encerrando devido a erro no carregamento do CSV.")
else:
    turmas, professores, salas, restricoes_globais = generate_example_data()
